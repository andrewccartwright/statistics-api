...
import com.andrewchatch.statisticsapi.exceptions.InvalidProbabilityException;
...
public class Geometric {
    ...
    private double calculateProbability(int x, double p) {
        double prob;
        if (includesSuccess) {
    ...
    private double calculateCumulativeProbInclusive(int x, double p) {
        double sum = 0;
        for (int i = 0; i <= this.x; i++) {
            sum += calculateProbability(x, p);
        }

        return sum;
    }

    private double calculateCumulativeProbExclusive(int x, double p) {
        double sum = 0;
        for (int i = 0; i < this.x; i++) {
            sum += calculateProbability(x, p);
        }
    ...
    public HashMap<String,Double> getProbability(int x, double p) {
        double pEqX = calculateProbability(x, p);
        double cumulativePInclusive = calculateCumulativeProbInclusive(x, p);
        double cumulativeQInclusive = 1 - cumulativePInclusive;
        double cumulativePExclusive = calculateCumulativeProbExclusive(x, p);
